...
class VisionTransformerLongLoRAStudent(BaseModule):
        ...
        **kwargs
        ...
        for i in range(depth):
            if i not in drop_idx:
                ...
                block_tmp.append(nn.Identity())
        self.blocks = ModuleList(block_tmp)
        ...
        self.new_depth = depth - len(drop_idx)
    ...
    def forward(self, x: Tensor, **kwargs) -> Tensor:
        ...
        if 'test' not in kwargs:
            ...
            if self.cal_loss_out:
                loss_backbone.update({'loss_out':loss_out / self.new_depth})
            if self.cal_loss_mid_rel:
                loss_backbone.update({'loss_mid_rel':loss_mid_rel / self.new_depth})
            if self.cal_loss_out_rel:
                loss_backbone.update({'loss_out_rel':loss_out_rel / self.new_depth})
