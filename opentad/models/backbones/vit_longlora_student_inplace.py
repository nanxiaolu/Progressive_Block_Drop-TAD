...
class Block(BaseModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not inplace:
            ...
            self.lora = LoRALayer(embed_dims, embed_dims, lora_rank)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.with_cp and x.requires_grad:
            feat_mid, x = cp.checkpoint(_inner_forward, x)
        else:
            feat_mid, x = _inner_forward(x)
        return feat_mid, x



@MODELS.register_module()
class VisionTransformerLongLoRAStudentInplace(BaseModule):
    ...
    def __init__(
        ...
        cal_loss_mid=False,
        cal_loss_out=False,
        cal_loss_mid_rel=False,
        cal_loss_out_rel=False,
        **kwargs
    ) -> None:
        if pretrained:
        ...
        self.inplace_idx = inplace_idx
    
        self.cal_loss_mid = cal_loss_mid
        self.cal_loss_out = cal_loss_out
        self.cal_loss_mid_rel = cal_loss_mid_rel
        self.cal_loss_out_rel = cal_loss_out_rel
        self.mes_loss = nn.MSELoss()
        self.mes_loss_sum = nn.MSELoss(reduction='sum')
        self.new_depth = depth - len(inplace_idx)
        self.depth = depth
    ...
    def forward(self, x: Tensor, **kwargs) -> Tensor:
        ...
        for idx, blk in enumerate(self.blocks):
            if idx in self.drop_idx: continue
            if idx not in self.inplace_idx:
                feat_mid, x = blk(x)
                if 'test' not in kwargs:
                    if self.cal_loss_mid:
                        loss_mid += self.mes_loss(feat_mid, kwargs['feat_mid_list'][idx])
                    if self.cal_loss_out:
                        loss_out += self.mes_loss(x, kwargs['feat_out_list'][idx])
                    if self.cal_loss_mid_rel:
                        loss_mid_rel += self.cal_rel_loss(feat_mid, kwargs['feat_mid_list'][idx])
                    if self.cal_loss_out_rel:
                        loss_out_rel += self.cal_rel_loss(x, kwargs['feat_out_list'][idx])
            else:
                _, x = blk(x)
                if 'test' not in kwargs:
                    if self.cal_loss_out:
        ...
        if 'test' not in kwargs:
            if self.cal_loss_mid:
                loss_backbone.update({'loss_mid':loss_mid / self.new_depth})
            if self.cal_loss_out:
                loss_backbone.update({'loss_out':loss_out / self.depth})
            if self.cal_loss_mid_rel:
                loss_backbone.update({'loss_mid_rel':loss_mid_rel / self.new_depth})
            if self.cal_loss_out_rel:
                loss_backbone.update({'loss_out_rel':loss_out_rel / self.depth})
        ...
        if 'test' not in kwargs:
            if self.return_feat_map:
                ...
                return x, loss_backbone
            if self.fc_norm is not None:
                return self.fc_norm(x.mean(1)), loss_backbone
            return x[:, 0], loss_backbone
        else:
            if self.return_feat_map:
                x = x.reshape(b, -1, h, w, self.embed_dims)
                x = x.permute(0, 4, 1, 2, 3)
    ...
    def cal_rel_loss(self, predict, target):
        loss = 0.
        predict_s = predict / predict.shape[1]
        target_s = target / target.shape[1]
        for i in range(predict.shape[0]):
            loss += self.mes_loss_sum(predict_s[i] @ predict_s[i].T, target_s[i] @ target_s[i].T)
        return loss / predict.shape[0]

    def set_drop_idx(self, drop_idx):
        self.drop_idx = drop_idx.copy()