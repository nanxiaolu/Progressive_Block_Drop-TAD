...
def get_backbone_optim_groups(cfg, model, logger):
    ...
    if 'train_block_idx' in cfg.keys():
        train_block_idx = cfg['train_block_idx']
    else:
        train_block_idx = []
    ...
    for name, param in model.module.backbone.named_parameters():
        ...
        if train_block_idx:
            is_train = False
            if 'patch_embed' in name or 'fc_norm' in name:
                is_train = True
            for i in train_block_idx:
        ...
        is_custom = False
        if len(custom_name_list) > 0 and is_train:
        ...
        # if is_exclude, we do not need to append the param to the rest_params_list
        if is_exclude or is_custom or not is_train:
            continue
