...
class ActionFormerTeacher(SingleStageDetector):
    def __init__(
        self,
        projection,
        rpn_head,
        ...
        self.cal_cls_loss = cal_cls_loss
        self.cal_reg_loss = cal_reg_loss
    ...
    def forward_train(self, inputs, masks, metas, gt_segments, gt_labels, **kwargs):
        x, feat_mid_list, feat_out_list = self.backbone(inputs, **kwargs)
        if not self.cal_cls_loss and not self.cal_reg_loss:
            return feat_mid_list, feat_out_list, None, None
        ...
        cls_pred, reg_pred = self.rpn_head.forward_train(
            x,
            masks,
        ...
        if not self.cal_cls_loss:
            return feat_mid_list, feat_out_list, None, reg_pred
        if not self.cal_reg_loss:
            return feat_mid_list, feat_out_list, cls_pred, None
        return feat_mid_list, feat_out_list, cls_pred, reg_pred
