...
class VisionTransformerTeacher(BaseModule):
    ...
    def forward(self, x: Tensor, **kwargs) -> Tensor:
        ...
        for idx, blk in enumerate(self.blocks):
            if idx not in self.drop_idx:
                ...
                feat_out_list.append(x)
            else:
                feat_mid_list.append(None)
        ...
        if self.return_feat_map:
            ...
            return x, feat_mid_list, feat_out_list

        if self.fc_norm is not None:
            return self.fc_norm(x.mean(1)), feat_mid_list, feat_out_list

        return x[:, 0], feat_mid_list, feat_out_list
