...
model = dict(
    backbone=dict(
        ...
        backbone=dict(
            type="VisionTransformerAdapterLongLoRA",
            img_size=224,
            patch_size=16,
            embed_dims=384,
            depth=12,
        ...
        custom=dict(
            pretrain="/data/zengrh/jiaqi/ActionRobustness/opentad-pretrained/vit-small-p16_videomae-k400-pre_16x4x1_kinetics-400_my.pth",
...
optimizer = dict(
    ...
    backbone=dict(
        lr=0,
        weight_decay=0,
        custom=[dict(name="adapter", lr=2e-4, weight_decay=0.05), dict(name="lora", lr=5e-5, weight_decay=0.05), dict(name="norm", lr=5e-5, weight_decay=0.05), dict(name="patch_embed", lr=5e-5, weight_decay=0.05)],
        exclude=["backbone"],
    ),
)
...
work_dir = f"exps/thumos/adatad/e2e_actionformer_videomae_s_768x1_160_adapter_longlora/{optimizer['backbone']['custom'][0]['lr']}_{optimizer['backbone']['custom'][1]['lr']}"
# work_dir = "/data-store/chenxiaoyong/lora_opentad/exps/thumos/adatad/debug"
