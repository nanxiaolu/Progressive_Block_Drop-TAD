...
class VisionTransformerLongLoRADropBlockLocalFT(BaseModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.cal_frozen_layers(drop_idx, depth)
        self.blocks = ModuleList(
            [
                Block(
                    ...
                    with_cp=with_cp,
                    lora_rank=lora_rank,
                    add_lora=(i in self.add_lora_idx)
                )
    ...
    def cal_frozen_layers(self, drop_idx, depth) -> None:
        self.frozen_layers = []
        self.add_lora_idx = []
        for i in range(depth):
            if (i - 1) not in drop_idx and (i + 1) not in drop_idx:
                self.frozen_layers.append(i)
            else:
                self.add_lora_idx.append(i)
