...
model = dict(
    backbone=dict(
        ...
        backbone=dict(
            type="VisionTransformerLongLoRA",
            img_size=224,
            patch_size=16,
            embed_dims=384,
            depth=12,
...
optimizer = dict(
    ...
    backbone=dict(
        lr=0,
        weight_decay=0,
        custom=[dict(name="lora", lr=5e-5, weight_decay=0.05), dict(name="norm", lr=5e-5, weight_decay=0.05), dict(name="patch_embed", lr=5e-5, weight_decay=0.05)],
        exclude=["backbone"],
    ),
)
...
# work_dir = f"exps/thumos/adatad/e2e_actionformer_videomae_s_768x1_160_Longlora/{optimizer['backbone']['custom'][0]['lr']}_{workflow['end_epoch']}_{model['backbone']['backbone']['lora_rank']}"
